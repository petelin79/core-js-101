...
function getDigitalRoot(num) {
  if (num > 9) {
    return getDigitalRoot(num.toString().split('').reduce((n, c) => Number(n) + Number(c), 0));
  }
  return num;
  // throw new Error('Not implemented');
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 1; i < pathes.length; i += 1) {
    const filenameParts = pathes[i].split('/');

    for (let j = 0; j < parts.length; j += 1) {
      if (parts[j] !== filenameParts[j]) {
        if (j === 0) {
          return '';
        }
        parts.splice(j);
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < 3; i += 1) {
    for (let j = 0; j < 3; j += 1) {
      cols[j][i] = position[i][j];
      if (i === j) {
        diagonals[0].push(position[i][j]);
      }
      if (i === 2 - j) {
        diagonals[1].push(position[i][j]);
      }
    }
  }
